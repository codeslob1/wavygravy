                ...
                */
                ...
                WindowEvent::MouseInput { state, button, .. } => {
                    if button == &MouseButton::Left {
                        ...
                        if let Some(_prior) = prior_position {
                            let width = render_state.surface.config.width;
                            let height = render_state.surface.config.height;
                            chart.handle_mousedown(&prior_position, width as f64, height as f64);
                        }
                    }
                }
                WindowEvent::MouseWheel { delta, .. } => {
                    ...
                    if let Some(prior) = prior_position {
                        ...
                        */
                        let width = render_state.surface.config.width;
                        let height = render_state.surface.config.height;
                        chart.handle_mousewheel(exponent, &prior, width as f64, height as f64);
                    } else {
                ...
                WindowEvent::CursorLeft { .. } => {
                    let width = render_state.surface.config.width;
                    let height = render_state.surface.config.height;
                    chart.handle_mouseup(&prior_position, width as f64, height as f64);
                    prior_position = None;
                }
...
*/
        ...
        Event::RedrawRequested(_) => {
            ...
            //builder.append(&fragment, None); //Some(transform));
            if chart_shown {
                chart.draw_layer::<bool>(
                    &mut builder,
                    &mut simple_text,
                    &mut datas,
                    width as f64,
